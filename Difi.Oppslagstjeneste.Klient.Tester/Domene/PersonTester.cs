using System;
using System.Diagnostics.Eventing.Reader;
using System.Security.Cryptography.X509Certificates;
using System.Xml;
using Difi.Oppslagstjeneste.Klient.Domene;
using Microsoft.VisualStudio.TestTools.UnitTesting;

namespace Difi.Oppslagstjeneste.Klient.Tester.Domene
{
    [TestClass]
    public class PersonTester
    {
        /// <summary>
        /// Tester oppretting av personobjekt fra Xml. Når dette objektet testes blir også følgende klasser
        /// implisitt testet: Epostadresse, Kontaktinformasjon, Mobiltelefonnummer og Sikkerdigitalpostadresse.
        /// </summary>
        [TestMethod]
        public void OpprettPersonFraXmlSuksess()
        {
            var doc = new XmlDocument();
            doc.LoadXml(PersonXml());
            
            Person p = null;
            try
            {
                p = new Person(doc.DocumentElement);
            }
            catch { Assert.Fail(); }

            Assert.AreEqual("08077000292", p.Personidentifikator);
            Assert.IsFalse((bool)p.Reservasjon);
            Assert.AreEqual(Tilstand.AKTIV, p.Status);

            Assert.AreEqual("+4740485641", p.Kontaktinformasjon.Mobiltelefonnummer.Nummer);
            Assert.AreEqual(new DateTime(2015, 04, 14, 14, 35, 19), p.Kontaktinformasjon.Mobiltelefonnummer.SistOppdatert);
            Assert.AreEqual(new DateTime(2015, 04, 14, 14, 35, 19), p.Kontaktinformasjon.Mobiltelefonnummer.SistVerifisert);


            Assert.AreEqual("extfjo@vegvesen.no", p.Kontaktinformasjon.Epostadresse.Epost);
            Assert.AreEqual(new DateTime(2015, 04, 14, 14, 35, 30), p.Kontaktinformasjon.Epostadresse.SistOppdatert);
            Assert.AreEqual(new DateTime(2015, 04, 14, 14, 35, 30), p.Kontaktinformasjon.Epostadresse.SistVerifisert);

            Assert.AreEqual("id.porten.testuser#2346", p.SikkerDigitalPostAdresse.Postkasseadresse);
            Assert.AreEqual("984661185", p.SikkerDigitalPostAdresse.PostkasseleverandørAdresse);

            var certificateData = Convert.FromBase64String(@"");

            Assert.AreEqual(new X509Certificate2(certificateData), new X509Certificate2());
        }

        private static string PersonXml()
        {
            return
            "<ns2:Person xmlns:ns2=\"http://begrep.difi.no\">" +
                "<ns2:personidentifikator>08077000292</ns2:personidentifikator>" +
                    "<ns2:reservasjon>NEI</ns2:reservasjon>" +
                    "<ns2:status>AKTIV</ns2:status>" +
                    "<ns2:Kontaktinformasjon>" +
                        "<ns2:Mobiltelefonnummer sistOppdatert=\"2015-04-14T14:35:19.000+02:00\" sistVerifisert=\"2015-04-14T14:35:19.000+02:00\">+4740485641</ns2:Mobiltelefonnummer>" +
                        " <ns2:Epostadresse sistOppdatert=\"2015-04-14T14:35:30.000+02:00\" sistVerifisert=\"2015-04-14T14:35:30.000+02:00\">extfjo@vegvesen.no</ns2:Epostadresse>" +
                    "</ns2:Kontaktinformasjon>" +
                    "<ns2:SikkerDigitalPostAdresse>" +
                        "<ns2:postkasseadresse>id.porten.testuser#2346</ns2:postkasseadresse>" +
                        " <ns2:postkasseleverandoerAdresse>984661185</ns2:postkasseleverandoerAdresse>" +
                    "</ns2:SikkerDigitalPostAdresse>" +
                " <ns2:X509Sertifikat></ns2:X509Sertifikat>" +
            "</ns2:Person>";
        }
    }
}
